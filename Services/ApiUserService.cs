...
public sealed class ApiUserService : IUserService, IDisposable
{
    ...
    {
        EnsureCurrentUser(userId);
        using var response = Send(HttpMethod.Post, "api/attendance/present", new
        {
            meetingType,
    ...
    {
        using var response = Send(HttpMethod.Delete, $"api/admin/projects/{Uri.EscapeDataString(projectId)}");
        return response.IsSuccessStatusCode;
    }
